...
def calculate_ai_metrics(t_type, count, age, height_m, diameter_cm, health_score=100):
    ...
    height_factor  = min(height_m / (age * 0.4 + 0.5), 1.3)
    ...
    multiplier = max(0.5, min(multiplier, 1.4))
...
# ─────────────────────────────────────────────
# AI DARAXT TAHLILI (Google Gemini API)  ← O'ZGARTIRILDI
# ─────────────────────────────────────────────
def analyze_tree_with_ai(image_bytes, api_key, selected_species=None, mime_type="image/jpeg"):
    """Google Gemini 1.5 Flash yordamida daraxt rasmi tahlili (bepul tier)"""
    try:
        import google.generativeai as genai
    except ImportError:
        return {"error": "google-generativeai kutubxonasi o'rnatilmagan. 'pip install google-generativeai' buyrug'ini ishga tushiring."}
...
{{
  ...
  "verdict_reason": "yakuniy xulosa"
        ...
        # Rasmni Gemini uchun tayyorlash
        import PIL.Image
...
if "anthropic_key"  not in st.session_state: st.session_state.anthropic_key  = ""  # gemini key saqlanadi bu yerda
...
def main_app():
    ...
    with st.sidebar:
        ...
        st.divider()
        if api_ok:
            st.markdown("🟢 **Gemini AI: Faol**")
        else:
            st.markdown("🔴 **Gemini AI: Sozlanmagan**")
    ...
    if menu == "📊 Dashboard":
        ...
        <div class='satellite-box'>
            ...
            🤖 AI TAHLIL ENGINE: Google Gemini 1.5 Flash (Computer Vision + Species ID + Fraud Detection)<br>
    ...
    # ════════════════════════════════════════════
        ...
        api_ok = len(st.session_state.anthropic_key) > 20
        if not api_ok:
            st.warning("⚠️ AI tahlil uchun **⚙️ AI Sozlamalari** bo'limiga borib Google Gemini API kalitingizni kiriting.")
        ...
        if photo is not None:
            ...
            if api_ok:
                ...
                <div class='progress-indicator'>
                    ...
                    <strong>Gemini AI daraxtni tahlil qilmoqda…</strong>
                </div>
                """, unsafe_allow_html=True)
    ...
    # ════════════════════════════════════════════
        ...
        with col3:
            ...
            k_health = st.slider("Sog'liq (%):", min_value=10, max_value=100, value=100, key="k_hs")
    ...
    # ════════════════════════════════════════════
    #  AI SOZLAMALARI  ← O'ZGARTIRILDI: Google Gemini
    # ════════════════════════════════════════════
    elif menu == "⚙️ AI Sozlamalari":
        st.title("⚙️ AI Sozlamalari — Google Gemini")
        st.markdown("**Gemini 1.5 Flash** — bepul tier, vision qo'llab-quvvatlaydi, tezkor va kuchli.")

        st.markdown("""
        <div class='ai-result-box'>
            🤖 CarbonVision AI imkoniyatlari (Gemini 1.5 Flash):<br>
            ...
            • 💰 Aqlli narx tavsiyasi (Smart Marketplace)<br><br>
            ✅ Gemini 1.5 Flash: <strong>Bepul tier</strong> — oyiga 1,500 so'rov / kuniga 1M token
        </div>
        """, unsafe_allow_html=True)

        st.divider()
        st.subheader("🔑 Google Gemini API Kaliti")
        st.markdown("""
        **API kalitini olish (bepul, 1 daqiqa):**
        1. [aistudio.google.com](https://aistudio.google.com/app/apikey) ga kiring
        2. **Get API Key** → **Create API key** tugmasini bosing
        3. Kalitni nusxalab, quyidagi maydonga joylashtiring
        """)
        ...
        api_key_input = st.text_input(
            ...
            type="password",
            placeholder="AIzaSy…",
            help="https://aistudio.google.com/app/apikey dan bepul oling"
        )
        ...
        with col_save:
            if st.button("💾 Saqlash", use_container_width=True):
                ...
                if len(api_key_input) > 20:
                    st.success("✅ Gemini API kalit saqlandi!")
                else:
            ...
            if st.button("🧪 Test qilish", use_container_width=True):
                if len(api_key_input) < 20:
                    ...
                    try:
                        ...
                        model = genai.GenerativeModel("gemini-1.5-flash")
                        resp = model.generate_content("Say: Gemini AI ready")
                        st.success(f"✅ Gemini ulandi: {resp.text.strip()[:60]}")
                    except Exception as e:
        ...
        st.divider()
        st.subheader("🧠 AI modeli: gemini-1.5-flash")
        st.markdown("""
        | Xususiyat | Ma'lumot |
        |-----------|---------|
        | **Narx** | ✅ Bepul (1,500 req/kun) |
        | **Vision** | ✅ Rasm tahlili to'liq |
        | **Tezlik** | ⚡ Juda tez (~1-2 sek) |
    ...
    # ════════════════════════════════════════════
        ...
        |----------|-------------|--------|
        | **Daraxt turi aniqlash** | Gemini 1.5 Flash Vision | Rasmdan avtomatik tur tanish |
        | **Rasm autentikligi** | EXIF + Gemini AI | Kamera meta, GPS, tahrirlash izlari |
        | **AI generatsiya tekshiruvi** | Gemini 1.5 Flash | Midjourney/DALL-E rasmlarini bloklash |
